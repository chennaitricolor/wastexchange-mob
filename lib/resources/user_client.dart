...
class UserClient {
  ...
  Future<LoginResponse> login(LoginData loginData) async {
    final String response =
        await _apiBaseHelper.post(PATH_LOGIN, loginData.toMap());
  ...
  Future<RegistrationResponse> register(RegistrationData data) async {
    final String response =
        await _apiBaseHelper.post(PATH_REGISTER, data.toJson());
  ...
  Future<OtpResponse> sendOTP(OtpData otpData) async {
    final String response =
        await _apiBaseHelper.post(PATH_SEND_OTP, otpData.toMap());
    final otpResponse = otpResponseFromJson(response);
    return otpResponse;
  }

  Future<List<User>> getAllUsers() async {
    final response = await _apiBaseHelper.get(PATH_USERS);
