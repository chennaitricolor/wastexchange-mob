...
void main() {
  ...
  setUp(() {
     ...
     userRepository =  UserRepository(client: userClient, tokenRepository: tokenRepository);
  });

  test('GIVEN otp data WHEN email and mobile number are valid THEN receive success response', () async {
    const message = 'OTP has been sent successfully';
    final otpData = OtpData(emailId: 'email', mobileNo: 'mobile');
    when(userClient.sendOTP(otpData)).thenAnswer((_) async => OtpResponse(message: message));
    userRepository.sendOTP(otpData).then((data) {
      expect(data.message, message);
    });
  });

  test('GIVEN registration data WHEN all the fields are valid THEN receive success response', () async {
    const message = 'registration message';
    final registrationData = RegistrationData(city: 'city', emailId: 'email', persona: 'persona', long: 14.5,
    lat: 10.2, password: '123445', altMobNo: 0, name: 'name', address: 'address',
    mobNo: 9988776655, pinCode: 123456, otp: 232345);

    when(userClient.register(registrationData)).thenAnswer((_) async => RegistrationResponse(message: message, success: true,
        auth: true, token: 'token_id'));
  ...
  test('GIVEN login data WHEN email and password are valid THEN receive success response', () async {
    final loginData = LoginData(loginId: 'email', password: '123445');

    when(userClient.login(loginData)).thenAnswer((_) async => LoginResponse(auth: true, token: 'token_id'));
    userRepository.login(loginData).then((data) {
      expect(data.auth, true);
      expect(data.token, 'token_id');
    });
  });
  ...
  test('GET all users THEN receive success response', () async {
    ...
    when(userClient.getAllUsers()).thenAnswer((_) async => [User(id: 1, pinCode: 123456,
    mobNo: mobileNumber, address: 'address', name: 'name', altMobNo: mobileNumber,
    lat: latitude, long: longitude, persona: 'seller', emailId: 'email', city: 'city', approved: true)]);
